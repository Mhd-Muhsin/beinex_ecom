...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  Widget build(BuildContext context) {
    CartBloc cartBloc = BlocProvider.of<CartBloc>(context, listen: true);
    return Scaffold(
      appBar: AppBar(
        actions: [
          BADGE.Badge(
            position: BADGE.BadgePosition.topEnd(top: 0, end: 3),
            showBadge: cartBloc.cartCount != 0 ? true : false,
            badgeContent: Text("${cartBloc.cartCount}", style: TextStyle(color: whiteColor, fontSize: 12),),
            child: IconButton(
            ...
            children: [
              Expanded(
                child: ListView(
                  ...
                  children: [
                    AspectRatio(aspectRatio: 1, child: Image.network(widget.product.image, fit: BoxFit.contain, errorBuilder: (_,_,_) => Icon(Icons.image, size: 100, color: purple,))),
              ...
              Padding(
                ...
                child: ElevatedButton(
                  onPressed: stock<=0 ? null : () {
                    ...
                    cartBloc.add(AddToCartEvent(id: widget.product.id, quantity: qty));
                    setState(() {
                      stock = stock - qty;
                    });
